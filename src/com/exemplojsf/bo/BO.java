...
import com.exemplojsf.connection.GenericDAO;
import com.exemplojsf.entity.PerfilObjeto;
import com.exemplojsf.exception.BancoDadosException;
import com.exemplojsf.exception.IntegridadeReferencialException;
import com.exemplojsf.exception.RegistroExistenteException;
...
public class BO<E extends Serializable> implements Serializable {
	...
	public BO() {
		this.persistenceUnitName = GenericDAO.PU_EXEJSF;
	}
	...
	protected String tratarCamposIn(List<String> lista) {
	...
	protected String tratarCamposInMenu(List<PerfilObjeto> perfilObjetos) {
		if (perfilObjetos != null && !perfilObjetos.isEmpty()) {
			String out = "";
			for (PerfilObjeto po : perfilObjetos) {
				out += "," + po.getObjeto().getMenu().getId();
			}
			out = out.replaceFirst(",", "");
			return out;
		}
